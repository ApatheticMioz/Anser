...
| F1, F2 | 27/45 dispatches over 1,500-char budget; monolithic mega-prompt failure | M7 `prompt_over_budget` advisory event at run() start (runner = only event-sink layer; anser_runner has none), `PROMPT_BUDGET_CHARS=1500` knob (env). Advisory only | [RESOLVED: this commit] 4/4 canary (over/under/boundary/override), full gate exit 0 |
| F17, F18 | Exporter: `promptTokens` all-zero; session-cumulative fields double-count reused sessions (~3×) | M8: exporter never committed (output-only in `e57ebd1`) — rebuilt as `scripts/qwen_tasks_analysis.mjs` (no-dep ESM CLI): promptTokens = MAX per-turn depth (null when unknown, never 0), cumulative fields stamped on last row only, rateAvgTokS derived from (totalMs−ttftMs) when engine field absent. Real-data proof: paper_p3 26.9M→196,455 tokens; 1,926→214 turns. Standalone test `tests/qwen_tasks_exporter.test.js` (repo-root tests/, package-boundary rationale in scripts/README.md) | [RESOLVED: this commit] test PASS + live smoke (72 dispatches / 53 sessions, matches audit window) |
| N2 | Mid-session death froze `task_anomaly-probe-s1` as `running`, unhealed (boot-only sweep) | M9 `reapOrphans()` in task_registry: not-done + status queued/running + heartbeat stale > `ORPHAN_REAP_STALE_MS` (1h, env) + dead ownerPid → reuses boot sweep's `markTaskOrphanedOnDisk` (same terminal marker); live-owner invariant absolute; fresh-heartbeat conservative skip; piggybacks the 5-min `cleanOldTasks` interval, no new timer | [RESOLVED: this commit] 27/27 canary (dead-PID reap, live-PID untouched, conservative skip, idempotency, cadence), full gate exit 0 |
...
| M7 | prompt_budget.test.js 4/4; full `npm test` exit 0 | pending MCP-server-restart smoke (over-budget dispatch → event in ledger) | this commit |
| M8 | qwen_tasks_exporter.test.js PASS; live run vs ~/.qwen: 72 rows / 53 sessions, corrected fields verified | live run done (real data) | this commit |
| M9 | orphan_reaper.test.js 27/27; orphan_terminal_event 31/31 no-regression; full `npm test` exit 0 | pending live smoke: the frozen `task_anomaly-probe-s1` row should reap on next status-server cycle | this commit |
| D1 | n/a | n/a | — |
