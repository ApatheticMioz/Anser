# Upstream issue — syv-ai/qwen38-27b-rtx3090

> Status: FINAL (verified against sources 2026-08-29). Post as an issue, not a PR.
> Verification notes in "## Notes for us" at the bottom.

---

**Title:** Engine core stalls on first large chunked-prefill after boot (WSL2, KVarN)
...
- PR candidate (only after maintainer reply): extend pool-init warmup to the
  prefill-continuation shapes (real-shape launches of both kernels).
- Do NOT re-apply the defensive kernel patch on this stack without bare-metal
  testing — it made the stall deterministic.
- VLLM_DFLASH2_CHAIN: dropped entirely (phantom variable, issue #38 feature
  never installed) — nothing to tell upstream about it.
- The 4.5h incident (08-28) was a 60–90k-token session; the A/B uses 24k.
  The 4.5h figure is the 14:51→19:21 stats gap; the core then unwedged on its
  own, so the issue says "spontaneously unwedged; operator stopped the
  engine" rather than "stayed wedged until reboot".
- Driver is 616.56; vLLM 0.27.1 confirmed via `import vllm`; 2ae239f confirmed
  via `git rev-parse`.
