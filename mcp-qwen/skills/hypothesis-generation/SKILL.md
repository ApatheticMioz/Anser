...
description: AVO discipline — state a measurable hypothesis, snapshot, evaluate, then select or revert.
keywords: [hypothesis, avo, fitness, benchmark, optimization, candidate, lineage]
---
# Hypothesis Generation (AVO Discipline)
...
2. Snapshot the files you are about to mutate with `avo_propose_candidate`
   BEFORE editing (pass the exact file list).
3. Make the minimal change that tests the mechanism — one variable at a time.
4. Run the verification with `avo_evaluate_candidate` (the test command).
5. Read the fitness score and the compact failure digest.
6. If it improved: `avo_select_candidate`. If it regressed or failed:
   `avo_revert_candidate` to restore the snapshot.
