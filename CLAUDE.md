...
- **Local Worker Responsibilities (Qwen via Goose & MCP @ $0)**:
  ...
  - Framework & library documentation lookup (`extensions: ['npx.cmd -y context7@latest']` or `['npx -y context7@latest']`).
...
2. **Synchronous Execution Model**:
   > [!IMPORTANT]
   > `qwen_coworker` is an atomic, synchronous tool. It executes to completion inside Goose and returns the complete deliverable directly in the same turn.
...
- **`qwen_coworker`**: The Primary Agent Interface. Executes multi-turn Socratic collaboration, codebase exploration, threat modeling, deep web research, and candidate variations inside Goose. Runs synchronously and returns complete deliverables directly in one turn.
  - Parameters: `prompt`, `session_id?`, `cwd?`, `extensions?`, `hypothesis?`, `test_command?`, `metric_name?`, `higher_is_better?`, `timeout_ms?` (default 3,600,000ms / 1 hour with stream heartbeat).
